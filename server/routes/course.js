...
router.post('/add', auth.isMentor, (req, res) => {
  ...
  if (errors) {
    ...
    new Course(newCourse)
      .save()
      .then((course) => {
        req.flash('success_msg', 'New Book submitted successfully. Add Description.');
...
router.put('/edit/:id', auth.isMentor, (req, res) => {
  ...
  if (errors) {
    ...
    Course.findByIdAndUpdate(req.params.id, newCourse)
      .then((course) => {
        req.flash('success_msg', 'Book edited successfully.');
...
//Editing a Lesson
//Rendering form
router.get('/lesson/edit/:id', auth.isOwner, (req, res) => {
  Lesson.findById(req.params.id)
    .then((lesson) => {
      res.render('body/mentor/addLesson', {
        layout: 'dashboard',
...
router.put('/lesson/edit/:id', auth.isOwner, (req, res) => {
  var {
    errors,
    newLesson
  } = validate.lesson(req);
  if (errors) {
    ...
    Lesson.findByIdAndUpdate(req.params.id, newLesson)
      .then((lesson) => {
        req.flash('success_msg', 'Description edited successfully.');
...
router.get('/approve/:id', auth.isAdmin, (req, res) => {
  ...
  Course.findByIdAndUpdate(req.params.id, {
    ...
    transporter.sendMail({
      from: `Team Books4You <${process.env.EMAIL}>`,
      to: mentor.email,
    ...
    req.flash('success_msg', 'Book Approved');
